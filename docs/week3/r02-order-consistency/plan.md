...
- [ ] 포인트 충전은 변경용 포인트 조회 후 충전·저장·기록 생성을 수행한다.
- [ ] 상품 수정·개별 삭제·최종 재고 설정은 변경용 상품 조회 후 판단·저장한다. 현재 mapper가 재고를 포함한 전체 값을 반영하므로 세 경로에 같은 규칙을 적용한다.
- [ ] 브랜드 일괄 삭제는 R01의 LEFT JOIN FETCH와 브랜드 단위 저장을 유지한다.
- [ ] 일괄 삭제에서 조회한 상품은 ID 오름차순으로 `refresh`와 비관적 쓰기 잠금을 적용한 뒤 도메인으로 변환한다. 이미 적재된 상품 상태를 최신 DB 값으로 갱신한다.
- [ ] 잠금 획득 전에 상품을 변경하거나 저장하지 않는다. 보호된 상품 상태를 삭제 처리와 저장까지 유지한다.
- [ ] 단위 테스트로 변경용 조회 사용을, MySQL 통합 테스트로 실제 잠금과 무관한 값 보존을 검증한다.
- [ ] R01 일괄 삭제·전체 롤백·삭제 후 확정 거절 테스트가 통과하면 커밋한다.

**완료 기준:** 운영 코드에서 같은 재고·잔액을 저장하는 경로가 판단 전 보호 규칙을 지킨다. 브랜드 삭제와 상품 등록의 동시 실행은 기존 제외 범위를 유지한다.

## 커밋 5 — 실제 SQL 이후 주문 확정 전체 롤백

**목표:** 변경 SQL이 실행된 이후 실패해도 이번 확정의 모든 변경이 취소됨을 증명한다.

- [ ] 여러 품목·기존 거래 기록·무관한 주문을 포함하는 데이터를 먼저 커밋한다.
- [ ] 테스트 전용 Writer spy에서 실제 저장을 호출한 뒤 flush한다. 같은 트랜잭션의 JDBC 조회로 변경 SQL의 반영을 확인하고 런타임 예외를 발생시킨다.
- [ ] 실제 Spring 서비스 프록시를 호출하고, 테스트 전체를 부모 트랜잭션으로 감싸지 않는다.
- [ ] 예외가 서비스 밖으로 전파된 뒤 별도 DB 조회로 DRAFT·전체 재고·잔액·기존 기록 보존과 신규 USE·PAID 기록 부재를 확인한다.
- [ ] 제품 코드에 실패 주입 분기·직접 복구·예외 삼키기를 추가하지 않는다.
- [ ] 관련 롤백·정상 확정 테스트가 통과하면 커밋한다.

**완료 기준:** 저장 전 검증 실패와 구분되는, 실제 SQL 실행 이후 전체 롤백 증거가 있다.

## 커밋 6 — 갱신 유실 대조군과 동시 실행 테스트 기반

**목표:** 보호 없는 갱신 유실을 재현하고 이후 경쟁 검증에 사용할 실행·결과 수집 기반을 준비한다.

- [ ] 기존 MySQL 8.0 fixture에서 독립된 두 connection·transaction이 재고 5를 읽도록 한다.
- [ ] 두 읽기 결과가 모두 5임을 확인한 뒤 테스트 전용 장벽을 해제한다.
- [ ] 각 트랜잭션이 상수 4를 저장하고 커밋하도록 한다. 성공 2·최종 재고 4와 수량식 불일치를 정상 assertion으로 검증한다.
- [ ] 대조군은 테스트 소스에만 두고 실제 서비스의 정합성 증거와 구분한다.
- [ ] worker 실행 도구는 요청별 성공·업무 거절·기술 오류를 수집한다. 준비 데이터 커밋, 시작 동기화, 제한 시간, finally 정리를 공통화한다.
- [ ] 시작 대기는 10초, 전체 결과 수집은 공통 마감 시각 기준 30초로 제한한다. 실패 시 대기를 해제하고 작업 취소·executor 종료·connection 반환을 수행한다.
- [ ] 대조군과 자원 정리 검증이 통과하면 커밋한다.

**완료 기준:** 의도적으로 실패하는 테스트 없이 갱신 유실을 설명하고, 모든 worker 결과와 종료를 추적할 수 있다.

## 커밋 7 — 실제 서비스 동시성 검증

**목표:** 실제 Spring 서비스의 성공·거절 결과와 최종 DB 상태가 일치함을 증명한다.

- [ ] 같은 주문 2회 확정: 성공 1·기존 상태 오류 1, 차감·USE·PAID 기록 각각 한 번.
- [ ] 재고 5에 서로 다른 사용자 주문 8건: 성공 5·재고 부족 3·최종 재고 0.
- [ ] 잔액 10,000원에 4,000원 주문 3건: 성공 2·잔액 부족 1·최종 잔액 2,000원.
- [ ] 잔액 10,000원에서 2,000원 충전과 7,000원 확정: 둘 다 성공·최종 잔액 5,000원.
- [ ] 초기 재고 5에서 최종 재고 10 설정과 수량 2 주문: 둘 다 성공하며 최종 재고는 순차 실행에 해당하는 8 또는 10.
- [ ] 동일한 두 상품을 반대 품목 순서로 가진 주문도 함께 성공하는지 확인해 상품 잠금 순서를 검증한다.
- [ ] 상품 정보 수정·개별 삭제·브랜드 일괄 삭제와 확정의 경쟁도 검증한다. 정보 수정은 성공 차감 보존, 삭제는 확정 선행 또는 삭제 선행에 해당하는 상태·기록만 허용한다.
- [ ] 모든 사례에서 기술 오류 0, 요청 집계 일치, 성공량에 따른 수량·잔액식, 거절 요청의 부분 반영 부재를 확인한다.
- [ ] 실제 서비스 테스트는 시작만 동기화한다. 제품 코드의 잠금 구간에 장벽·sleep을 넣거나 통과할 때까지 재시도하지 않는다.

**완료 기준:** 필수 경쟁 결과와 다른 저장 경로의 보호를 실제 DB의 주문·품목·잔액·재고·기록으로 확인한다.
