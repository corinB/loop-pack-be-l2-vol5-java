...
상태: 커밋 1(Point→Wallet 전환)·커밋 2(주문 확정 도메인 규칙 분리)·커밋 3(JPA 잠금 조회·저장)·커밋 4(같은 행을 변경하는 기존 경로 보호)·커밋 5(실제 SQL 이후 전체 롤백)·커밋 6(갱신 유실 대조군과 동시 실행 테스트 기반) 구현·테스트 통과. 커밋 7~8은 미착수다. R01 병합 내용은 현재 작업 브랜치에 반영되어 있다.
