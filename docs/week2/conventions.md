...
학습 단계는 [개발 계획](development-plan.md)을 따른다. 과제 원문의 인증·인가는 이번 범위에서 제외한다.
...
기존 ID·생성 시각·잠금 버전 등 저장 메타데이터를 새 기본값으로 덮어쓰지 않는다.
...
어느 저장에서든 실패하면 전체 롤백한다. Mapper 분리가 중복 차감이나 동시성 보호를 대신하지 않는다.
