...
계획 정리는 PR 01에 포함한다. 현재 문서 보완은 구현·검사 완료를 의미하지 않으며 아래 구현 항목은 모두 미완료다.
...
| 02. 브랜드·상품 운영과 조회 | Money·Stock, Brand·Product, 관리자 CRUD·재고 설정, 고객 상품·브랜드 조회 | 브랜드 삭제 제한, 논리 삭제, 값 범위, 필터·정렬·페이지·응답 조합 | 14 | R01~R03, R10~R12 |
| 03. 좋아요 | 등록·취소·사용자별 목록, 상품 집계 연결 검증 | 반복 요청 성공, 사용자별 관계, 삭제 상품 처리, 실제 좋아요 수·정렬 | 3 | R04, R05 |
...
- **상품의 좋아요 수:** PR 02에 Shopping 소유의 Like 저장 구조·`(userId, productId)` 유일성 제약과 COUNT 조회를 준비한다. 테스트는 관계 데이터를 직접 저장해 집계를 검증한다. 등록·취소·목록 유스케이스는 PR 03에서 연결한다.
...
- **구조:** 순수 domain과 JPA Entity 분리, 계층별 전달 객체, 명시적 save, application의 트랜잭션 조율을 유지한다.

상품 조회는 실제 Like COUNT를 사용하며 집계·정렬 후 페이지를 적용한다.
...
| 좋아요·조회 | 반복 요청, 사용자별 관계, 실제 COUNT, 동률 정렬, 빈 페이지·없는 브랜드 필터, 전체 개수 |
...
| 02 | 값 객체 → 브랜드 → 상품·재고·Like 저장 구조 → 조회 |
...
- [ ] PR 01: 계획·작업 규칙·공통 기반·사용자 입력 완료 및 병합
