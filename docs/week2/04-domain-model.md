...
| Like | userId·productId의 유효성 | 상품 확인, 관계 유일성; 사용자 존재는 resolver에서 검사 |
...
Shopping의 상품별 좋아요 집계 테이블은 조회용 파생 데이터이며 Product Aggregate의 상태가 아니다.
관계 변경과 별도 트랜잭션에서 시작 시·10초 간격으로 전체 갱신한다. 실패 시 이전 집계 값을 유지한다.
