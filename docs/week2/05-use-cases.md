...
각 행동은 `ConfirmOrderUseCase` 같은 인터페이스와 `ConfirmOrderService` 같은 구현체로 나눈다.
실행 메서드는 `execute`이며, 쓰기·조회 트랜잭션은 각각 구현체에서 일반·readOnly로 선언한다.
입력·결과는 Request → Command/Criteria → Result → Response로 분리한다. [명명·변환 규칙](conventions.md)을 따른다.
...
| 좋아요 등록 | 헤더 사용자 존재 확인 → 활성 상품 확인 → 관계가 없을 때만 저장 | 관계 변화 없음 |
| 좋아요 취소 | 헤더 사용자 존재 확인 → 해당 관계가 있으면 제거 | 관계 없으면 그대로 성공 |
| 포인트 충전 | 헤더 사용자 존재 확인 → 금액·합산 범위 검사 → 잔액 증가·CHARGE 기록 | 잔액·기록 유지 |
| 주문 생성 | 입력·사용자 존재 검사 → 중복 수량 합산 → 활성 상품 조회 → 스냅샷·합계 저장 | 주문·품목 생성 없음 |
