...
class PointApiE2ETest {
    ...
    class Charge {
        ...
        @Test
        void chargesPoint() {
            createUserWithPoint(1L);

            ResponseEntity<ApiResponse<PointApiDto.BalanceResponse>> response = charge(1_000L, "1");

            assertAll(
                () -> assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK),
                () -> assertThat(response.getBody().data().balance()).isEqualTo(1_000L),
                () -> assertThat(pointRepository.findByUserId(1L).orElseThrow().getBalance()).isEqualTo(1_000L),
        ...
        void returnsBadRequest_whenBalanceOverflows() {
            ...
            pointRepository.save(Point.restore(1L, Long.MAX_VALUE));

            ResponseEntity<ApiResponse<Object>> response = chargeRaw(1L, "1");

            assertAll(
                () -> assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST),
                () -> assertThat(pointRepository.findByUserId(1L).orElseThrow().getBalance()).isEqualTo(Long.MAX_VALUE),
                () -> assertThat(countChargeBills(1L)).isZero()
            );
        }
    ...
    private ResponseEntity<ApiResponse<PointApiDto.BalanceResponse>> charge(long amount, String userIdHeader) {
