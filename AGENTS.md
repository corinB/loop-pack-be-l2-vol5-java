...
학습 범위와 단계별 완료 조건은 [개발 계획](docs/week2/development-plan.md)을 따른다.
브랜치는 `volume-2/<작업영역>` 형식을 사용한다. `volume-2/base`에 docs, common, Context, integration 브랜치를
검증 후 순서대로 병합하며 Context 브랜치끼리 직접 병합하지 않는다.
